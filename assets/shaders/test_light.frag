...
vec3 directionalLight(directionalSource ls, material m, vec3 normal, vec3 view_dir) {
    ...
    vec3 tmp = ls.color.rgb;
    
    vec3 col = tmp; //vec3(texture(m.diffuse, uv_coords));
    vec3 ambient = vec3(ls.ambient) * col;
    vec3 diffuse = vec3(ls.diffuse) * col * max(dot(normal, light_ray), 0.0);
    vec3 specular = vec3(ls.specular) * pow(max(dot(view_dir, refl), 0.0), m.shininess) * tmp; //vec3(texture(m.specular, uv_coords));

    return (ambient + diffuse + specular);
}

vec3 pointLight(lightSource ls, material m, vec3 normal, vec3 vert_coords, vec3 view_dir) {
    vec3 light_ray = -normalize(light.position - vert_coords);
    ...
    vec3 specular = ls.specular * pow(max(dot(view_dir, refl), 0.0), m.shininess) * tmp; //vec3(texture(m.specular, uv_coords));
